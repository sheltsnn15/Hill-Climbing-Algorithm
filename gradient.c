...
void add_noise(float **matrix, int height, int width, int number, int max_size)
{
    ...
    for (i = 0; i < number; i++)
    {
        ...
        if (landscape_peak.x > startx && landscape_peak.x < endx && landscape_peak.y > starty && landscape_peak.y < endy)
        {
            continue;
        }
        ...
        for (y = starty; y < endy; y++)
        {
            for (x = startx; x < endx; x++)
            {
                ...
                int distance = (center_x - x) * (center_x - x) + (center_y - y) * (center_y - y);
...
void generate_view(float view[VIEW_SIZE][VIEW_SIZE], int center_y, int center_x)
{
    ...
    if (queries_made > landscape_width * landscape_height)
...
int declare_peak(int x, int y)
{
    ...
    queries_made++;
    if (x == landscape_peak.x && y == landscape_peak.y)
        return 1;
    else
        return 0;
}
